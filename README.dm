...
This WTI Device Upgrade program will query WTI devices for their version and device type, then go online to see if there is a newer version available. If there is a new version will be downloaded and the WTI device updated.

This `Device Upgrade` Python script will work on any modern WTI device, the device upgrade Automated calls are universal on all WTI OOB and PDU type devices.
...
The program will ask for the protocol "http or https", the location of your WTI device and the username and password.
